...
def trackOneClip(
    ...
    tracker_trace_length,
    no_ant_counter_frames_total,
    edge_border,
    merge_distance,
    model_weights_path,
    model_device="cuda",
    ...
    # create ML detector object (TorchVision Faster R-CNN)
    detector_object = MLDetector(
        MLDetectorConfig(
    ...
    # don't touch first_go. without it, you might get exit times that are less than entry times
    first_go = False
    ...
    while True:
        ...
        cv2.putText(frame, str(current_timestamp), (0, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 2)
        ...
        for i in range(len(tracker_object.tracks)):
            try:
                ...
                # if the ant disappeared near the edge, we will give it no_ant_counter_frames_total frames to reappear
                if x_bound_left <= place_last_seen[0] <= x_bound_right and y_bound_bottom <= place_last_seen[1] <= y_bound_top:
                    gone_frames = no_ant_counter_frames_total * 1
                else:
                    gone_frames = no_ant_counter_frames_total * 2
                ...
                if frame_counter - tracker_object.tracks[i].frame_last_seen > gone_frames:
                    ...
                    if x_bound_left <= place_last_seen[0] <= x_bound_right and y_bound_bottom <= place_last_seen[1] <= y_bound_top:
                        ...
                        tracker_object.tracks[i].appear_middle_end = True
                ...
                for i in range(len(tracker_object.tracks)):
                    if len(tracker_object.tracks[i].trace) > 1:
                        for j in range(len(tracker_object.tracks[i].trace) - 1):
                            ...
                            cv2.line(frame, (int(x1_trace), int(y1_trace)), (int(x2), int(y2)), track_colors[clr], 1)
...
def make_history_CSV(tracker_object, history_path):
    ...
    for history in tracker_object.histories:
        # separate different times with a space (not a comma, remember we are using a csv)
        ...
        middle_begin = 1 if history.appear_middle_begin is True else 0
        middle_end = 1 if history.appear_middle_end is True else 0

        # append row by row
        full_list_history.append(
            [
                history.filename,
                history.id,
                history.x0,
                ...
                unmerge_id,
                unmerge_time,
                history.number_warning,
                history.broken_track,
            ]
        )
        ...
        for row in reader:
            ...
            if len(merge_time) > 0 and len(unmerge_time) > 0:
                ...
                for unmerge in unmerge_time:
                    ...
                    first_one = True
                    range_of_times.append((floor(begin_time) - 5, ceil(unmerge) + 5))  # add 5 seconds of padding
                ...
                if range_of_times != []:
                    for index, time in enumerate(range_of_times):
                        ...
                        final_result_path = os.path.join(result_path, f"ID_{id}_part_{index}.mp4")
                        final_annotated_result_path = os.path.join(annotated_result_path, f"ID_{id}_part_{index}.mp4")

                        # Create the directories if they don't exist
                        os.makedirs(os.path.dirname(final_result_path), exist_ok=True)
                        os.makedirs(os.path.dirname(final_annotated_result_path), exist_ok=True)

                        # Process the videos using FFmpeg
                        ffmpeg_command = (
                            f"ffmpeg -y -i {video_source} -ss {from_time} -to {to_time} -c copy -loglevel error {final_result_path}"
                        )
