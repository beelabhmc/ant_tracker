...
# IMPORTANT: this tracker reads timing/frame globals from the clip driver module.
# The legacy tracker uses `track_one_clip`; the ML pipeline uses `track_one_clip_new`.
import track_one_clip_new as track_one_clip
...
class History:
    ...
    def __init__(self, prediction, trackIdCount):  # note that track_id and index for self.histories is the exact same
        ...
        self.merge_list = []  # contains ant IDs this ant may have merged with
...
class Tracker:
    ...
    def Update(self, centers, areas):
        ...
        # Deletion of tracks done in track_one_clip_new
            ...
            if self.tracks[i].change_in_area_max >= self.tracks[i].median_area // 2:
                ...
                for j in range(len(self.tracks)):
                    if fps > track_one_clip.frame_counter - self.tracks[j].frame_last_seen > fps // 4:
                        if len(self.tracks[j].trace) == 0:
                            print(
                                f"WARNING: In search of merger for Ant {self.tracks[i].track_id}, no trace for Ant  {self.tracks[j].track_id}"
                            )
                        else:
                            ...
                            if distance < self.merge_distance:
                                if (
                                    self.tracks[j].track_id not in self.tracks[i].merge_list
                                    and self.tracks[i].track_id != self.tracks[j].track_id
                                ):
                                    ...
                                    print(
                                        f"MERGER: Ant {self.tracks[i].track_id} got bigger and Ant {self.tracks[j].track_id} got absorbed! "
                                        "merge set",
                                        self.tracks[i].merge_list,
                                        "time",
                                        self.tracks[i].merge_time,
                                    )

            # detecting potential unmergers
            if self.tracks[i].change_in_area_min <= -1 * self.tracks[i].median_area // 4:
                try:
                    change_location = tuple(int(value[0]) for value in self.tracks[i].trace[-1])
                    for j in range(len(self.tracks)):
                        if fps > track_one_clip.frame_counter - self.tracks[j].frame_first_seen > fps // 4:
                            lost_location = tuple(int(value[0]) for value in self.tracks[j].trace[-1])

                            x, y = abs(change_location[0] - lost_location[0]), abs(change_location[1] - lost_location[1])
                            distance = sqrt(x**2 + y**2)

                            if distance < self.merge_distance * 2 and self.tracks[i].attached_to_me > 0:
                                if (
                                    self.tracks[j].track_id not in self.tracks[i].unmerge_list
                                    and self.tracks[i].track_id != self.tracks[j].track_id
                                ):
                                    ...
                                    if self.tracks[i].attached_to_me > 0:
                                        ...
                                        print(
                                            "WARNING: ant",
                                            self.tracks[i].begin_middle,
                                            "unmerged even though no merger was detected",
                                        )

                                    if self.tracks[i].attached_to_me == 0:
                                        self.tracks[i].last_unmerge_time = track_one_clip.current_timestamp

                                        track_one_clip.merge_times.append(
                                            (self.tracks[i].first_merge_time, self.tracks[i].last_unmerge_time)
                                        )
                                        self.tracks[i].first_merge_time = -1
                                        self.tracks[i].last_unmerge_time = -1

                                    print(
                                        f"UNMERGER: Ant {self.tracks[i].track_id} got smaller and Ant {self.tracks[j].track_id} appeared! "
                                        "unmerge set",
                                        self.tracks[i].unmerge_list,
                                        "time",
                                        self.tracks[i].unmerge_time,
                                    )
                except Exception:
                    pass
