...
def load_data():
    # Try to find all annotation JSON files under this `ml/` folder and load them all
    base_dir = Path(__file__).parent  # this file is in ml/
    ann_candidates = list(base_dir.rglob("*annotations*.json"))

    datasets = []
    if ann_candidates:
        for ann in ann_candidates:
            # Prefer an "images" subfolder if present, otherwise use the annotation's parent directory
    ...
    # Fallback to the single hard-coded dataset if none found or all failed
    if not datasets:
        print("No annotation candidates found under ml/. Falling back to the default single dataset path.")
